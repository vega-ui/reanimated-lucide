import * as React from "react";
import type { SVGProps } from "react";
import { type SvgProps } from "react-native-svg";
import { useAnimatedProps } from "react-native-reanimated";
import { AnimatedSvg, AnimatedPath } from "../../animatedFactory";
export interface AnimatedSvgProps extends SvgProps {
  size?: number | string;
  color?: string;
  strokeWidth?: number | string;
  svgProps?: Partial<React.ComponentProps<typeof AnimatedSvg>>;
  pathProps?: Partial<React.ComponentProps<typeof AnimatedPath>>;
  animatedProps?: ReturnType<typeof useAnimatedProps>;
  ref?: React.Ref<React.Component<SvgProps>>;
}
const SvgTriangleDashed: React.FC<AnimatedSvgProps> = ({
  size = 24,
  color = "currentColor",
  strokeWidth = 2,
  svgProps,
  pathProps,
  animatedProps,
  ref,
  ...props
}) => (
  <AnimatedSvg
    viewBox="0 0 24 24"
    fill="none"
    strokeLinecap="round"
    strokeLinejoin="round"
    animatedProps={animatedProps}
    ref={ref}
    width={size}
    height={size}
    stroke={color}
    strokeWidth={strokeWidth}
    {...props}
  >
    <AnimatedPath
      d="M10.17 4.193a2 2 0 0 1 3.666.013"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath d="M14 21h2" animatedProps={animatedProps} {...pathProps} />
    <AnimatedPath
      d="m15.874 7.743 1 1.732"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath
      d="m18.849 12.952 1 1.732"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath
      d="M21.824 18.18a2 2 0 0 1-1.835 2.824"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath
      d="M4.024 21a2 2 0 0 1-1.839-2.839"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath
      d="m5.136 12.952-1 1.732"
      animatedProps={animatedProps}
      {...pathProps}
    />
    <AnimatedPath d="M8 21h2" animatedProps={animatedProps} {...pathProps} />
    <AnimatedPath
      d="m8.102 7.743-1 1.732"
      animatedProps={animatedProps}
      {...pathProps}
    />
  </AnimatedSvg>
);
export default SvgTriangleDashed;
